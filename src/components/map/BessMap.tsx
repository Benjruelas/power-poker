...
function setParcelLayerVisibility(
  ...
  for (const id of PARCEL_FILL_LAYERS) {
    setVis(map, id, show);
  }
  for (const id of PARCEL_LINE_LAYERS) {
    setVis(map, id, show);
  }
  // Halo only helps on imagery; keep it off for the light basemap
  for (const id of PARCEL_LINE_HALO_LAYERS) {
    setVis(map, id, show && satellite);
  }
}
...
function queryParcelAtLngLat(
  ...
  if (!layers.length || map.getZoom() < PARCEL_LAYER_MIN_ZOOM) {
    return null;
  }
...
function parcelLookupPoint(
  raw: Record<string, unknown>,
  fallbackLat: number,
  fallbackLng: number
): { lat: number; lng: number } {
  const cx = Number(raw.centroidx ?? raw.lon ?? raw.LONGITUDE);
  const cy = Number(raw.centroidy ?? raw.lat ?? raw.LATITUDE);
...
function streetTileUrls(): string[] {
  const origin =
    typeof window !== "undefined" ? window.location.origin : "";
  // Same-origin proxy — avoids CORS and Carto's basemap API-key requirement.
  return [`${origin}/api/esri-tiles?layer=street-base&z={z}&x={x}&y={y}`];
}
...
function streetLabelTileUrls(): string[] {
  const origin =
    typeof window !== "undefined" ? window.location.origin : "";
  return [`${origin}/api/esri-tiles?layer=street-labels&z={z}&x={x}&y={y}`];
}
...
const STREET_ATTR = "Esri, OpenStreetMap, Garmin, USGS";
...
function ensurePlaceLabels(map: maplibregl.Map, satellite: boolean) {
  const tiles = satellite ? satLabelTileUrls() : streetLabelTileUrls();
...
function buildMapStyle(satellite: boolean): maplibregl.StyleSpecification {
  return {
    ...
    sources: {
      basemap: {
        type: "raster",
        tiles: satellite ? satTileUrls() : streetTileUrls(),
        tileSize: 256,
        maxzoom: BASEMAP_MAXZOOM,
...
function applyBasemap(map: maplibregl.Map, satellite: boolean) {
  ...
  if (src && typeof src.setTiles === "function") {
    src.setTiles(satellite ? satTileUrls() : streetTileUrls());
  }
...
export function BessMap({
  ...
  useEffect(() => {
    ...
    map.on("mousemove", (e) => {
      ...
      if (
        ...
        map.getZoom() >= PARCEL_LAYER_MIN_ZOOM &&
        parcelLayers.length
      ) {
  ...
  selectParcelAtRef.current = (lng, lat, feat, knownLrid) => {
    ...
    const finishWithTileFallback = () => {
      ...
      const fallback = buildParcelViews(
        mapped,
        lookup.lat,
        lookup.lng,
        lrid,
    ...
    fetchLandRecordsParcel({
      lat: lookup.lat,
      lng: lookup.lng,
      lrid: lrid || (raw.lrid != null ? String(raw.lrid) : undefined),
      signal: ac.signal,
    })
      .then((result) => {
        ...
        const merged = mergeParcelProperties(mapped, result.properties);
